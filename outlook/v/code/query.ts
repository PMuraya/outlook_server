...
export class crosstab extends view.view{
    ...
    *get_cte():Generator<string>{  
    ...
    get_summaries(){
        ...
        const right_margin: HTMLElement | null = document.querySelector('details > details > fieldset');

        // Get all the checkboxes within the right margin fieldset
        const checkboxes: NodeListOf<HTMLInputElement> = right_margin!.querySelectorAll('input[type="checkbox"]');

        // Create an array to store the checked items
        const checked_items: string[] = [];
        ...
        checked_items.forEach((item: string) => {
          switch (item) {
            case 'total':
